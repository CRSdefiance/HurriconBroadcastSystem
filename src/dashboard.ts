...
import type { BroadcastImageAsset } from './asset-library';
...
const matchRep=observe<MatchState>('match',mockMatch,(m)=>{
  all<HTMLInputElement>('[data-field]').forEach((el)=>{if(document.activeElement!==el)el.value=String(m[el.dataset.field as 'game'|'round'|'bestOf']??'')});
  (['player1','player2'] as const).forEach((side,i)=>{all<HTMLInputElement>(`[data-p${i+1}]`).forEach((el)=>{if(document.activeElement!==el)el.value=String(m[side][el.dataset[`p${i+1}`] as keyof typeof m[typeof side]]??'')});const out=$<HTMLOutputElement>(`[data-score-value="${side}"]`);if(out)out.value=String(m[side].score);setText(`[data-live-player="${side}"]`,m[side].displayName)});
  all<HTMLButtonElement>('[data-status]').forEach((button)=>{const active=button.dataset.status===m.status;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active));});
});
...
const renderSponsorEditors = (sponsors: SponsorItem[]) => {
  ...
  box.innerHTML = sponsorDraft.map((_, index) => `<article class="sponsor-editor" data-sponsor-editor="${index}"><div class="sponsor-editor-head"><h3>Sponsor ${index + 1}</h3><button type="button" data-remove-sponsor="${index}" class="danger">Remove</button></div><label class="check"><input type="checkbox" data-sponsor-field="enabled"> Include in rotation</label><label>Display name<input maxlength="120" data-sponsor-field="name"></label><label>Logo image<div class="asset-field"><input maxlength="500" data-sponsor-field="logoUrl" placeholder="Choose an asset, HTTPS URL, or /bundles/…"><button type="button" data-pick-asset>Select asset</button></div></label></article>`).join('');
...
$('[data-action="apply"]')?.addEventListener('click',()=>{const current=matchRep.get();const readPlayer=(side:'player1'|'player2',number:1|2)=>({...current[side],displayName:value(`[data-p${number}="displayName"]`),pronouns:value(`[data-p${number}="pronouns"]`),location:value(`[data-p${number}="location"]`),social:value(`[data-p${number}="social"]`),socialPlatform:(value(`[data-p${number}="socialPlatform"]`)||'twitch') as SocialPlatform,seed:Number(value(`[data-p${number}="seed"]`))||null});matchRep.set({...current,game:value('[data-field="game"]'),round:value('[data-field="round"]'),bestOf:Number(value('[data-field="bestOf"]'))||null,player1:readPlayer('player1',1),player2:readPlayer('player2',2)});toast('Match info applied');});
all<HTMLButtonElement>('[data-status]').forEach((button)=>button.addEventListener('click',()=>{const status=button.dataset.status as MatchState['status'];matchRep.set({...matchRep.get(),status});toast(status==='complete'?'Match marked complete':`Match status: ${status}`);}));
...
const renderInterstitialEditors = (slides: InterstitialSlide[]) => {
  ...
  if (!box || box.contains(document.activeElement)) return;
  box.innerHTML = interstitialDraft.map((_, index) => `<article class="interstitial-slide-editor" data-interstitial-slide="${index}"><div class="interstitial-slide-head"><h3>Slide ${index + 1}</h3><button class="danger" data-remove-interstitial-slide="${index}">Remove</button></div><label class="check"><input type="checkbox" data-slide-field="enabled"> Include in rotation</label><label>Eyebrow / type<input data-slide-field="kicker" maxlength="100"></label><label>Headline<input data-slide-field="title" maxlength="180"></label><label>Body<textarea data-slide-field="body" maxlength="500" rows="3"></textarea></label><label>Slide image<div class="asset-field"><input data-slide-field="imageUrl" maxlength="1000" placeholder="Choose an asset, HTTPS URL, or /bundles/…"><button type="button" data-pick-asset>Select asset</button></div></label><label>Image treatment<select data-slide-field="imageFit"><option value="contain">Fit whole image (logos and posters)</option><option value="cover">Fill frame (photos; may crop)</option></select></label></article>`).join('');
...
$('[data-action="add-interstitial-slide"]')?.addEventListener('click', () => { if (interstitialDraft.length >= 24) return toast('Slide limit is 24'); interstitialDirty = true; interstitialDraft.push({ id: `slide-${Date.now()}`, enabled: true, title: 'New event message', imageFit: 'contain' }); renderInterstitialEditors(interstitialDraft); });
...
$('[data-action="apply-interstitial"]')?.addEventListener('click', () => { const slides = all<HTMLElement>('[data-interstitial-slide]').map((panel, index) => { const field = (name:string) => (panel.querySelector<HTMLInputElement|HTMLTextAreaElement|HTMLSelectElement>(`[data-slide-field="${name}"]`)?.value ?? '').trim(); return { id: interstitialDraft[index]?.id || `slide-${index + 1}`, enabled: panel.querySelector<HTMLInputElement>('[data-slide-field="enabled"]')?.checked ?? true, kicker: field('kicker'), title: field('title'), body: field('body'), imageUrl: field('imageUrl') || undefined, imageFit: field('imageFit') === 'cover' ? 'cover' as const : 'contain' as const }; }).filter((slide) => slide.title); const next = normalizeInterstitial({ ...interstitialState, automatic: $<HTMLInputElement>('[data-interstitial-setting="automatic"]')?.checked ?? true, rotationSeconds: Number(value('[data-interstitial-setting="rotationSeconds"]')) || 12, slides, updatedAt: Date.now() }); interstitialDirty = false; if (window.nodecg) void window.nodecg.sendMessage('interstitial:update', next); else interstitialRep.set(next); toast('Interstitial applied'); });
...
// form field, avoiding manual path copying during show setup.
const installAssetPicker = () => {
  if (!$('[data-brand-select]')) return;
  let assets: BroadcastImageAsset[] = [];
  ...
  const render = () => {
    ...
    assets.forEach((asset) => {
      const button = document.createElement('button'); button.type = 'button'; button.className = 'asset-choice'; button.title = `Use ${asset.base}`;
      const image = document.createElement('img'); image.src = asset.url; image.alt = '';
      const name = document.createElement('strong'); name.textContent = asset.base;
      button.append(image, name); button.addEventListener('click', () => choose(asset.url)); grid.append(button);
    });
  };
  if (window.nodecg) {
    const rep = window.nodecg.Replicant<BroadcastImageAsset[]>('assets:images');
    rep.on('change', (next) => { assets = Array.isArray(next) ? next : []; render(); });
    if (Array.isArray(rep.value)) assets = rep.value;
  }
