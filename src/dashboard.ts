...
type HbsPreset = { version: 1; name: string; savedAt: string; match: MatchState; speedrun: SpeedrunState; broadcastRail: BroadcastRailState; lowerThird: LowerThirdState; show: ShowState; transitionSettings: TransitionSettings; activeBrand: string };
const presetCard = document.createElement('section');
presetCard.className = 'card preset-card';
presetCard.innerHTML = '<span class="eyebrow">Configuration library</span><h2>Save / load setup</h2><p class="hint">Save match data, layouts, runner identities, rail sponsors, lower thirds, programming, transitions, and the active brand as a reusable JSON preset.</p><div class="two"><label>Preset name<input data-preset-name maxlength="80" placeholder="Default tournament"></label><div class="actions"><button data-preset-save class="primary">Save to file</button><button data-preset-load>Load from file</button><input data-preset-file type="file" accept="application/json,.json" hidden></div></div><p class="message" data-preset-message></p>';
